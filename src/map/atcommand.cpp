...
ACMD_FUNC(go)
{
	...
	const struct {
		...
		{ MAP_MORROC,        160,  95 }, //  1=Morroc
		{ MAP_GEFFEN,        119,  59 }, //  2=Geffen
		{ MAP_PAYON,         173,  101}, //  3=Payon
		{ MAP_ALBERTA,        30, 235 }, //  4=Alberta
#ifdef RENEWAL
		{ MAP_IZLUDE,        128, 142 }, //  5=Izlude (Renewal)
#else
		{ MAP_IZLUDE,        128, 142 }, //  5=Izlude
#endif
		{ MAP_ALDEBARAN,     140, 130 }, //  6=Al de Baran
		{ MAP_LUTIE,         148, 130 }, //  7=Lutie
		{ MAP_COMODO,        189, 150 }, //  8=Comodo
		{ MAP_YUNO,          157,  51 }, //  9=Yuno
		{ MAP_AMATSU,        198,  84 }, // 10=Amatsu
		{ MAP_GONRYUN,       160, 115 }, // 11=Gonryun
		{ MAP_UMBALA,         96, 152 }, // 12=Umbala
		{ MAP_NIFLHEIM,      197, 178 }, // 13=Niflheim
		{ MAP_LOUYANG,       217,  40 }, // 14=Louyang
#ifdef RENEWAL
		{ MAP_NOVICE,         53, 111 }, // 15=Training Grounds (Renewal)
#else
		{ MAP_NOVICE,         53, 111 }, // 15=Training Grounds
#endif
		{ MAP_JAIL,           23,  61 }, // 16=Prison
		{ MAP_JAWAII,        249, 127 }, // 17=Jawaii
		{ MAP_AYOTHAYA,      207, 261 }, // 18=Ayothaya
		{ MAP_EINBROCH,      232, 190 }, // 19=Einbroch
		{ MAP_LIGHTHALZEN,   158,  92 }, // 20=Lighthalzen
		{ MAP_EINBECH,       176, 121 }, // 21=Einbech
		...
		{ MAP_ECLAGE_IN,      47,  54 }, // 35=Eclage
		{ MAP_LASAGNA,       170, 152 }, // 36=Lasagna
		{ MAP_MOC_PARA01,     30,  13 }, // 37=Eden
		{ MAP_BAT_ROOM,      154, 150 }, // 38=BG
		{ MAP_QUE_TEMSKY,     48, 135 }, // 39=Sky Garden
		{ MAP_PARA_MARKET,    98,  81 }, // 40=Para Market
		{ MAP_VERUS04,       121, 249 }, // 41=Verus
		{ MAP_HARBORO1,      284, 194 }, // 42=Rock Ridge
		{ MAP_ARCHER_VILLAGE, 50, 130 }, // 43=Archer Village
		{ MAP_INSIDE_NIFLHEIM,31, 171 }, // 44=Inside Niflheim
		{ MAP_THOR_CAMP,     245,  70 }, // 45=Thor Camp
		{ MAP_FREYA_TEMPLE,  165, 134 }, // 46=Freya Temple
		{ MAP_PRT_MONK, 	 182, 205 }, // 47=St. Capitolina Abbey
		{ MAP_CMD_FILD07, 	 255, 115 }, // 48=Beacon Island
	};
	...
	if (strncmp(map_name, "prontera", 3) == 0) {
		...
		town = 38;
	} else if (strncmp(map_name, "sky garden", 2) == 0) {
		town = 39;
	} else if (strncmp(map_name, "para market", 2) == 0) {
		town = 40;
	} else if (strncmp(map_name, "verus", 2) == 0) {
		town = 41;
	} else if (strncmp(map_name, "rock ridge", 2) == 0) {
		town = 42;
	} else if (strncmp(map_name, "archer village", 2) == 0) {
		town = 43;
	} else if (strncmp(map_name, "inside niflheim", 2) == 0) {
		town = 44;
	} else if (strncmp(map_name, "thor camp", 2) == 0) {
		town = 45;
	} else if (strncmp(map_name, "freya temple", 2) == 0) {
		town = 46;
	} else if (strncmp(map_name, "st. capitolina abbey", 2) == 0) {
		town = 47;
	} else if (strncmp(map_name, "Beacon Island", 2) == 0) {
		town = 48;
	}
